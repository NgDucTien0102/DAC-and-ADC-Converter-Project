...
class ADCFrame(ttk.Frame):
    ...
    ROUND_NEAREST = "Làm tròn gần nhất"
    ROUND_FLOOR = "Làm tròn xuống (rút bớt)"
    ...
    def __init__(self, master, back_callback):
        ...
        self.r_var = tk.StringVar(value="1.0")
        self.mode_var = tk.StringVar(value=self.MODE_UNIPOLAR)
        self.rounding_var = tk.StringVar(value=self.ROUND_NEAREST)

        self.status_var = tk.StringVar(value="Nhập số bit, x, R, mode mã hóa và kiểu làm tròn rồi nhấn Convert.")
    ...
    def round_half_up(v: float) -> int:
        """Làm tròn gần nhất, nếu đúng chính giữa thì lên mức trên."""
        return math.floor(v + 0.5)

    def apply_rounding(self, v: float, rounding_mode: str) -> int:
        """
        ROUND_NEAREST: chọn mức gần nhất, trường hợp .5 lên trên.
        ROUND_FLOOR: chọn mức gần nhất nhưng không lớn hơn giá trị ngõ vào.
        """
        if rounding_mode == self.ROUND_NEAREST:
            return self.round_half_up(v)
        if rounding_mode == self.ROUND_FLOOR:
            return math.floor(v)
        raise ValueError("Kiểu làm tròn không hợp lệ.")

    def adc_quantize(self, x, R, bits, mode, rounding_mode):
        Q = R / (1 << bits)
        n = 1 << bits
        ...
        if mode == self.MODE_UNIPOLAR:
            ...
            m = self.apply_rounding(x / Q, rounding_mode)
            ...
            m = self.apply_rounding((x + R / 2) / Q, rounding_mode)
            m = self.clamp(m, 0, n - 1)
            xq = Q * m - R / 2
            signed_str = "-"
            ...
            s = self.apply_rounding(x / Q, rounding_mode)
    ...
    def on_convert(self, event=None):
        ...
        m, Q, xq, signed_str, rng, quant_err = self.adc_quantize(
            x, R, b, self.mode_var.get(), self.rounding_var.get()
        )
        if m is None:
        ...
        self.set_status(f"Hợp lệ ✓ ({self.rounding_var.get()})", kind="ok")
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            self,
            text="Hỗ trợ Convert đến 64 bit. Miền x hợp lệ đổi theo mode, R và có thể chọn kiểu làm tròn."
        ).pack(anchor="w", pady=(0, 10))
        ...
        ttk.Label(inp, text="Kiểu làm tròn:").grid(row=2, column=2, sticky="w", pady=(10, 0))
        self.rounding_box = ttk.Combobox(
            inp,
            textvariable=self.rounding_var,
            values=[self.ROUND_NEAREST, self.ROUND_FLOOR],
            state="readonly",
            width=24,
        )
        self.rounding_box.grid(row=2, column=3, sticky="w", padx=(8, 0), pady=(10, 0))
        ...
        self.range_hint_label.grid(row=3, column=0, columnspan=4, sticky="w", pady=(10, 0))

        button_row = ttk.Frame(inp)
        button_row.grid(row=4, column=0, columnspan=4, sticky="e", pady=(12, 0))
